...
状态：工作包 `1-6` 的保护性切片已完成；当前 `AR-0 + SK-0` 未关闭，工作包 `7-10` 和新功能发布冻结
...
本地 `ARCH-GATE` 通过后，项目应具备（完整蓝图见[最终重构蓝图](../../architecture-target-blueprint-2026-08-26.md)）：

- 一个同源入口和一个 FastAPI 模块化业务单体，直接托管前端构建产物；内置 SQL runner 默认并发为 1。
- 同一个 MySQL 实例/数据库承载全部业务事实，FastAPI 是唯一写权威；Redis 不参与最终正确性。
- 原始文档、图片和 Skill 包进入 SQL；Chroma 只保存带 generation、可对账和可重建的 RAG projection。
- 长任务使用 SQL durable job、UoW、lease/fencing、重试、取消、DLQ 和背压。
...
| R2 平台与任务 | settings、UoW、统一 SQL migration、RAG ports、durable job/runner | migration 与局部 outbox 已有；SQL runner、重启恢复和统一 UoW 未完成 |
...
| R8 切换与部署 | 单一 FastAPI 入口、Django/Redis/旧文件权威退出、单机恢复；可选公网 canary/HA/DR | 当前仍是三进程过渡拓扑；公网部分属于 `PUBLIC-HA-GATE` |
