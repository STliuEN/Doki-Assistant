...
开发环境由 Vite proxy 聚合两个后端。虚线是当前 FastAPI 在认证状态缓存失效时对 Django 的临时依赖，属于过渡态；目标是在 FastAPI 接管认证后移除。生产环境目前没有单机部署定义。当前三进程拓扑是过渡态，目标拓扑见[最终重构蓝图](../../architecture-target-blueprint-2026-08-26.md)。
...
| 数据 | 当前位置/角色 | 重写目标 |
|------|---------------|----------|
| 用户与认证数据 | Django MySQL，当前权威 | 同一 MySQL 数据库 `users/auth_sessions/roles/revocations/audit`，由 FastAPI 写入 |
| 会话、笔记、记忆、知识元数据、模型配置 | FastAPI MySQL，当前业务权威 | 同一关系库 schema，由统一 migration 管理 |
| 缓存、token 撤销记录、认证状态短缓存、pending action | Redis，当前短期状态 | 迁移期可保留；最终会话/refresh/撤销进入 MySQL，Redis 不参与正确性并删除 |
| 知识库和笔记向量 | `backend/data/chromadb`，派生索引 | Chroma adapter projection，可按版本重建 |
| 上传源文件和解析图片 | `backend/data/` 与 MySQL Blob 多路状态 | 原始文档/图片进入同一 MySQL 业务表；文件只作显式 debug/import/export 通道 |
| Django 头像 | `DjangoUserService/media/` | 迁移到 MySQL 业务数据后删除 Django 文件权威 |
...
这些未完成项在[架构重写计划](../../architecture_rewrite_plan.md)和[最终重构蓝图](../../architecture-target-blueprint-2026-08-26.md)中维护唯一阶段状态，[产品路线图](./roadmap_next.md)只保留 R0-R8 职责和产品队列。当前先关闭 `AR-0 + SK-0`，再按统一 SQL、FastAPI 认证、SQL runner、Chroma projection 和 Skill/A/B 对账依赖推进。本地产品工作包 `7-10` 和新功能发布在 `ARCH-GATE` 前冻结；C 级执行和公网/HA 分别由 `EXEC-SKILL-GATE` 与 `PUBLIC-HA-GATE` 控制。
