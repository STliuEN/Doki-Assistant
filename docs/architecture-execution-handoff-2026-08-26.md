...
状态：`E0/S0 已关闭`（仅文档阶段；当前 `AR-0 + SK-0` 仍未通过）  
...
- 当前仍是 `AR-0 + SK-0`；P0-0 至 P0-5 只有当前环境和离线证据，P0-6 尚未通过。
- 下一位执行者必须从 **AR-0 剩余证据包**开始，不得直接做 AR-1、数据库迁移或旧依赖删除。
- 新功能、工作包 `7-10`、C 级 Skill、公网和 HA 全部冻结。
- 允许的当前工作只包括 P0/安全/数据完整性、真实隔离环境、门禁证据、文档和启动阻断。
...
| E1 | AR-0/SK-0（S0 之后的前置证据） | AR-0 + SK-0 | 真实依赖、故障注入、备份恢复和 characterization；未关闭 |
| E2 | S1 | AR-1 | 统一 MySQL schema、UoW、SQL durable job、单并发 runner、备份/恢复和迁移工具；不迁移现有业务数据 |
...
E0 文档交接与冻结（已关闭）
  ...
  -> E2/AR-1/S1 单 MySQL schema + SQL job/UoW/runner（并发 1）
...
未做：任何代码、迁移、删除、部署切换。  
交接动作：下一位负责人复制模板建立 E1 批次目录，填写 owner、approver、环境、备份位置和停线联系人；未填写前不得把 E1 标为 `实施中`。
...
### E2：AR-1/S1 单 MySQL schema、UoW、durable job 与内置 runner

入口：E1 关闭；隔离 MySQL 已批准。  
任务：
...
1. 复制[阶段执行记录模板](./stage-execution-record-template-2026-08-26.md)，建立 E1 批次目录，填写 owner/approver 和隔离环境申请。
2. 固定当前 Git commit、工作树状态和归档路径；不得清理或重写历史批次记录。
3. 获得隔离 MySQL/Chroma 拓扑和备份位置的用户批准；没有批准则只做 fixture/documentation，不做真实连接。
4. 执行 E1 故障注入、备份/恢复、characterization 和证据整理；完成后先标 `待验证`，等待用户关闭确认。
5. E1 未关闭前，不开始 E2/AR-1 实施，不创建或执行真实 migration，不解冻任何产品功能。

## 9. 交接完成判据

交接完成不是“代码已经重构”，而是下一位执行者可以无歧义地开始 E1，并知道何时必须停止。判据为：
