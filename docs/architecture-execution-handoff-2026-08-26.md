...
- E3/AR-2 于 2026-09-01 关闭；E4 于 2026-09-10 关闭，见[关闭证据](../project_changes/2026-09-02-e4-ar3-business-migration/artifacts/e4-closure-20260910.json)。E5 按持续执行/收口授权于 2026-09-14 完成全部退出证据并关闭，来源用户核定发生在测试后。中间材料和旧输入继续保留，E6/E7 已于 2026-09-15 联合关闭（本机 SQL 权威、A/有限 B 范围）。
...
| E5 | S4 | AR-4 | SQL 原文 + Chroma RAG projection；已关闭，C1–C6/V01–V09 通过，完整证据可追溯 |
| E6 | S5 | AR-5 / SK-1..3 | 与 E7 联合已关闭；10 包 SQL 迁移、稳定 ID/digest、8 授权启用，2 外部 MCP 保持 disabled/unsupported |
| E7 | S6 | AR-5 | 与 E6 联合已关闭；图文/笔记/聊天 SQL 回接、真实模型 HTTP/SSE、SQL-only 恢复及故障重启通过 |
...
E0 文档交接与冻结（已关闭）
  ...
  -> E5/AR-4/S4 SQL 原文 + Chroma RAG projection 收敛（已关闭）
  -> E6 + E7 / AR-5 联合批次（2026-09-15 已关闭；JV01–JV09 通过）
...
入口与执行：E2/E3/E4 已关闭，Q1–Q7、执行/收口授权及来源核定齐全。2026-09-14 完成新 E5 allowlist、即时 preflight、先备份后受控目标重建，目标为 `127.0.0.1:33427/doki_e4` 与隔离 Chroma。当前验收完整；历史备份顺序/preflight 偏差保留，不追认先前启动门槛。
...
2026-09-14 最终关闭：[E5 计划](../project_changes/2026-09-10-e5-ar4-rag-projection/plan.md)、[执行记录](../project_changes/2026-09-10-e5-ar4-rag-projection/execution-record.md)及[最终判定](../project_changes/2026-09-10-e5-ar4-rag-projection/closure-review.md)为交接入口。6 知识/7 笔记，57/11 chunks，两用户 4 active/0 staging；52 FK 无孤儿。真实模型质量、生命周期/故障、API/浏览器、恢复 SQL→全新 Chroma 和最终目标 smoke 通过。后端 529、前端 29/build 通过。C1–C6 无剩余阻塞，E6/E7 已于 2026-09-15 联合关闭（本机 SQL 权威、A/有限 B 范围）。

Q4–Q7 已确认：E5 应用层只暂停当前用户相关写入；知识/笔记全部成功后一起开放，任一纳入文档失败阻断，日常索引重建同样 503。不得使用 MySQL 全局只读、全库锁、主机停服务或修改 new-api 连接/权限。E5 已完成实施与验收，此边界对后续操作继续有效。

### E6：AR-5/S5/SK-1..3 Codex Skill 标准化、授权和发布

2026-09-14 用户已明确 E5 关闭并要求 E6/E7 伴生、一起执行。[联合准备和执行计划](../project_changes/2026-09-14-e6-e7-ar5-joint/plan.md)覆盖两个交付面；准备 105 项为历史基线；2026-09-15 迁移、管理员配置、联合退出验收已完成，后端 539、前端 29 及 lint/build 通过。见[最终判定](../project_changes/2026-09-14-e6-e7-ar5-joint/closure-review.md)。以下保留各自已履行责任。
...
联合结果：E6/E7 于 2026-09-15 同批关闭。共同 schema、包与授权接口、业务切换已完成；实际图文、笔记、模型聊天、用户隔离、审计、故障及 SQL-only 恢复证据见联合 contracts.md。以下为本次已验收任务。
任务：
...
当前交接完成判据是 E1–E7 关闭证据可追溯，E5 与 E6/E7 的历史偏差和实际范围透明，E8 及全局门禁的冻结边界明确。判据为：

- 唯一入口、目标拓扑、数据权威、RAG/Skill 合同和阶段顺序已固定。
- 每阶段的输入、产物、测试、回滚、删除顺序和关闭责任已写明。
- E5 C1–C6 与 E6/E7 JV01–JV09 已分别凭证据关闭；E8 尚未启动，未被前序关闭声明覆盖。
- 归档文档不再出现在 `docs/` 一级目录。
- 文档链接和格式检查通过。
