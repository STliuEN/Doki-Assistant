...
def main():
    ...
    while True:
        probe = subprocess.run(
            [
                ...
                "-h127.0.0.1",
                "-NBe",
                "SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name='doki_e4'",
            ],
            env=exec_env,
            capture_output=True,
        )
    ...
    with args.dump.open("rb") as dump_file:
        docker("exec", "-i", "--env", "MYSQL_PWD", name, "mysql", "-uroot", "--binary-mode=1", "doki_e4", stdin=dump_file, env=exec_env)

    def query(container, sql, env, user):
        return docker("exec", "--env", "MYSQL_PWD", container, "mysql", "-u" + user, "-NBe", sql, env=env).decode().strip()

    tables = query(
        name, "SELECT table_name FROM information_schema.tables WHERE table_schema='doki_e4' ORDER BY table_name", exec_env, "root"
    ).splitlines()
    ...
    if (current["Id"], current["State"]["StartedAt"], current["RestartCount"]) != (
        protected["Id"],
        protected["State"]["StartedAt"],
    ...
    record = {
        ...
        "table_counts": counts,
        "target_data_digest": target_digest,
        "restored_data_digest": restored_digest,
        "new_api_unchanged": True,
        "target_global_flags_observed": query(target, "SELECT @@global.read_only, @@global.super_read_only", target_env, "doki_e4_app"),
    }
