...
|---|---|---|---|---|---|
| API-01 | `GET /health/live`、`GET /health/ready` | FastAPI health router；Chroma 可为 ready/degraded | live 返回 envelope；ready 区分核心依赖与 Chroma projection 状态 | `health.py` 保留 `ready/degraded` 字段；Chroma API containment 与 probe 覆盖降级结构 | verified-local；完整 lifespan 受 MySQL schema 阻塞 |
...
| UI-04 | 前端静态回归 | Node 22.20.0/npm 10.9.3（显式 PATH） | tests/lint/build 全通过 | Vitest `6 files, 28 passed`；ESLint 和 production build 通过 | verified-local |
...
| PROMPT-04 | benchmark Skill/Tool 合同 | `skill_ids=["memory_write"]` 同时请求 `delete_memory` 等工具的 fixture | 若 Skill 未授权 Tool 应稳定拒绝或 fixture 明确授权 | smoke 4 cases 中 3 pass/1 error；regression 117 中 78 pass/39 error，集中在同一合同冲突 | blocked；需 fixture/合同决策，不在 E1 擅改 |
...
| ROUTE-03 | 应用启动 schema gate | 主应用 lifespan + 当前环境 DB | 只读校验 revision `20260824_0002`，不自动 migration | 完整 pytest `280 passed, 1 failed`；唯一失败为 schema revision/lifespan，E1 禁止 migration | blocked；需隔离批准的 schema 环境 |
